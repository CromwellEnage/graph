...
// comparison.
    ...
    template <
        typename IncidenceGraph, typename DFSVisitor,
    ...
    )
    {
        ...
        typedef std::pair<
            Vertex, std::pair<boost::optional<Edge>, std::pair<Iter, Iter> >
        > VertexInfo;
...
namespace boost { namespace graph {
    ...
    BOOST_PARAMETER_FUNCTION(
        ...
        (required
            (graph, *(boost::detail::argument_predicate<is_edge_list_graph>))
        )
        (deduced
            (required
                (edge_color_map
                  , *(
                        boost::detail::argument_with_graph_predicate<
                            boost::detail::is_edge_color_map_of_graph
                        >
                    )
                )
            )
    ...
    template <
        typename Graph, typename visitor_type, typename color_map_type,
...
#endif  // BOOST_GRAPH_CONFIG_CAN_DEDUCE_UNNAMED_ARGUMENTS
    {
        ...
        if (s != boost::detail::get_default_starting_vertex(graph))
        {
            ...
            boost::detail::undir_dfv_impl(
                graph, s, vis, color_map, edge_color_map
            );
        }
        ...
        for (boost::tie(ui, ui_end) = vertices(graph); ui != ui_end; ++ui)
        {
            ...
            if (u_color == Color::white())
            {
                ...
                boost::detail::undir_dfv_impl(
                    graph, *ui, vis, color_map, edge_color_map
                );
            }
        }
...
namespace boost { namespace graph {
    ...
    BOOST_PP_REPEAT_FROM_TO(
        1, 6, BOOST_GRAPH_PP_FUNCTION_OVERLOAD, undirected_dfs
    )
}} // namespace boost::graph
...
namespace boost { namespace graph {
                 ...
                 >::type = mpl::true_())
  {
    ...
    if (start_vertex != *vertices(g).first){ vis.start_vertex(start_vertex, g);
      boost::detail::undir_dfv_impl(g, start_vertex, vis, vertex_color, edge_color);
    }

    for (boost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui) {
      ColorValue u_color = get(vertex_color, *ui);
      if (u_color == Color::white()) {       vis.start_vertex(*ui, g);
        boost::detail::undir_dfv_impl(g, *ui, vis, vertex_color, edge_color);
      }
    }
  }
  ...
  {
    boost::detail::undir_dfv_impl(g, u, vis, vertex_color, edge_color);
  }
}} // namespace boost::graph
...
namespace boost { namespace graph {
    ...
    BOOST_GRAPH_MAKE_FORWARDING_FUNCTION(undirected_dfs, 1, 5)
...
namespace boost {
    ...
    using ::boost::graph::undirected_dfs;
    using ::boost::graph::undirected_depth_first_visit;
}
