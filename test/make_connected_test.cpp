...
int main(int, char* [])
{
  ...
  BOOST_TEST(connected_components(gVL, make_vector_property_map<int>(get(vertex_index,gVL)))
             == static_cast<int>(num_cycles)
             );
  make_connected(gVL);
  BOOST_TEST(connected_components(gVL, make_vector_property_map<int>(get(vertex_index,gVL)))
  ...
  BOOST_TEST(connected_components(gLL, make_vector_property_map<int>(get(vertex_index,gLL)))
             == static_cast<int>(num_cycles));
  make_connected(gLL);
  BOOST_TEST(connected_components(gLL, make_vector_property_map<int>(get(vertex_index,gLL)))
             == 1
